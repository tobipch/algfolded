/**
 * Generate ambiguous scrambles for the 2-flip and 2-twist sets
 * (src/assets/edge_flips.json, src/assets/corner_twists2.json). Same approach as
 ...
 * can be resumed. Set env FT_SET=flip|twist to restrict to one file, and
 * FT_LIMIT=N to cap how many (previously empty) cases are processed per run.
 *
 * Usage: node scripts/generate_flip_twist_scrambles.mjs
 */
...
const FILES = {
  flip: new URL("../src/assets/edge_flips.json", import.meta.url),
  twist: new URL("../src/assets/corner_twists2.json", import.meta.url),
};
...
async function main(){
  ...
  const only=process.env.FT_SET;
  const limit=process.env.FT_LIMIT?parseInt(process.env.FT_LIMIT,10):0;
