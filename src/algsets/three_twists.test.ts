...
import { partition, DEFAULT_BUFFER_ORDER, type RawTwist } from './three_twists'
...
const byId = (cases: ReturnType<typeof partition>) =>
  Object.fromEntries(cases.map((c) => [c.id, c.path]))
...
describe('three-twists partition', () => {
  it('matches the documented example: cw {UFR,UBL,UBR} -> C/CR/CRN', () => {
    ...
    expect(cases[0].path).toEqual(['C', 'CR', 'CRN'])
  })
  ...
  it('encodes direction in the letters: ccw of the same triple -> C/CE/CEQ', () => {
    ...
    expect(cases[0].path).toEqual(['C', 'CE', 'CEQ'])
  })
  ...
  it('assigns the buffer to the highest-priority corner in the triple', () => {
    ...
    expect(cases[0].path[0]).toBe('D') // UFL
  })
  ...
  it('re-partitions when the buffer order changes', () => {
    ...
    expect(withUFR.path[0]).toBe('C') // UFR is highest priority by default
    ...
    expect(without.path[0]).toBe('D') // UFL
  })
  ...
  it('handles triples containing a non-buffer corner (DBL/DBR)', () => {
    ...
    expect(cases[0].path[0]).toBe('B') // UBR
  })
  ...
  it('produces a unique case per twist (no duplicates)', () => {
    ...
    expect(Object.keys(byId(cases))).toHaveLength(1)
  })
})
